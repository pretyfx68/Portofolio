...
if(f==='trending')return real.filter(s=>(s.tags||[]).includes('trend'));
  ...
  if(f==='senang') return real.filter(s=>(s.tags||[]).includes('senang'));
  return real;
}
...
function czmRunNpMarquee(){
  ...
  const clipW = clip ? clip.getBoundingClientRect().width : 0;
  if(clipW === 0) return;
  ...
  if(currentTitle === _lastNpMarqueeTitle && animRunning) return;
...
/* Cari artistKey dari const artists yang cocok dengan nama artis lagu */
function czmFindArtistKey(artistName){
  if(typeof artists==='undefined')return null;
  if(artists[artistName])return artistName;
  const low=artistName.toLowerCase();
  const keys=Object.keys(artists);
  for(const k of keys){ if(low.includes(k.toLowerCase())||k.toLowerCase().includes(low))return k; }
  const tokens=low.split(/[\s,&]+/).filter(t=>t.length>=3);
  for(const t of tokens){ const k=keys.find(k=>k.toLowerCase().includes(t)||t.includes(k.toLowerCase()));if(k)return k; }
  return null;
}
...
window.czmDoSearch=function(q){
  ...
  if(!query){if(chips)chips.style.display='none';czmRenderRecentSearches();return;}
  ...
  /* CHIP: SEMUA — card artis + daftar lagu */
  if(czmSrchChipActive==='semua'){
    // Cari 1 artis paling relevan dari const artists
    let matchedKey=null;
    if(typeof artists!=='undefined'){
      const keys=Object.keys(artists);
      for(const k of keys){
        if(artists[k].name.toLowerCase().includes(query)||query.includes(k.toLowerCase())){matchedKey=k;break;}
      }
      if(!matchedKey)matchedKey=czmFindArtistKey(found[0]?.artist||'');
    }

    if(matchedKey&&typeof artists!=='undefined'&&artists[matchedKey]){
      const a=artists[matchedKey];
      const cardSongs=allSongs.filter(x=>czmFindArtistKey(x.artist||'')===matchedKey||x.artist===matchedKey).slice(0,3);
      const totalViews=cardSongs.reduce((s,x)=>s+(x.views||0),0);
      const meta=a.pendengar?`${a.pendengar} pendengar bulanan`:`${fv(totalViews)} pemutaran`;
      // encode key untuk onclick — pakai JSON.stringify agar aman semua karakter
      const keyJs=JSON.stringify(matchedKey);
      const qJs=JSON.stringify(q);

      html+=`<div class="czm-artist-card">
        <div class="czm-ac-header" onclick="czmSaveRecentQ(${qJs});czmCloseSearch();czmOpenArtistPage(${keyJs})">
          ...
          <div class="czm-ac-info">
            ...
            <div class="czm-ac-meta">${meta}</div>
          </div>
          <i class="fa-solid fa-chevron-right czm-ac-arrow"></i>
        </div>
        <div class="czm-ac-btns">
          <button class="czm-ac-btn czm-ac-btn-acak" onclick="event.stopPropagation();czmSaveRecentQ(${qJs});czmShuffleArtist(${keyJs});czmCloseSearch()">
            <i class="fa-solid fa-shuffle"></i> Acak
          </button>
          <button class="czm-ac-btn czm-ac-btn-radio" onclick="event.stopPropagation();czmSaveRecentQ(${qJs});czmPlayArtistFirst(${keyJs});czmCloseSearch()">
            <i class="fa-solid fa-circle-dot"></i> Radio
          </button>
        </div>
        <div class="czm-ac-songs">
          ${cardSongs.map(s=>{
            const idJs=JSON.stringify(s.id);
            return `<div class="czm-ac-song" onclick="event.stopPropagation();czmSaveRecentQ(${qJs});czmCloseSearch();czmPlayById(${idJs},true)">
              ...
              <button class="czm-q-more-btn" onclick="event.stopPropagation();czmOpenBs(${idJs},event)"><i class="fa-solid fa-ellipsis-vertical"></i></button>
            </div>`;
          }).join('')}
        </div>
      </div>`;
    ...
    // Daftar lagu di bawah card artis
    html+=`<div class="czm-srch-section-lbl" style="margin-top:8px">Lagu</div>`;
    html+=found.map(s=>{
      const idJs=JSON.stringify(s.id);const qJs2=JSON.stringify(q);
      return `<div class="czm-qitem" onclick="czmSaveRecentQ(${qJs2});czmCloseSearch();czmPlayById(${idJs},true)">
  ...
  /* CHIP: LAGU / TREND / SAD — hanya daftar lagu */
  if(czmSrchChipActive==='lagu'||czmSrchChipActive==='trend'||czmSrchChipActive==='sad'){
    html+=`<div class="czm-srch-section-lbl">Lagu</div>`;
    html+=found.map(s=>{
      const idJs=JSON.stringify(s.id);const qJs=JSON.stringify(q);
      return `<div class="czm-qitem" onclick="czmSaveRecentQ(${qJs});czmCloseSearch();czmPlayById(${idJs},true)">
        <img class="czm-q-thumb" src="${s.image}" loading="lazy">
        <div class="czm-q-info">
          <div class="czm-q-title">${s.title}</div>
          <div class="czm-q-sub">Lagu • ${s.artist} • ${fv(s.views)}</div>
...
// Render daftar lagu playlist di bawah player (full view)
function czmRenderPlayerPlBox(){
...
const artists = {
 ...
 "Rex Orange County": {
    ...
    image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/1_10b23a8e-d94f-42b9-a2a8-c080b87583e9.jpg",
    pendengar: "89.3 juta"
  },
  "Chris Brown": {
    name: "Chris Brown",
...
const songs = {
 ...
 97: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Happiness.mp3", 
       ...
       tags: ["sad","sedih"],
       views: 98500000},
 98: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Best%20Friend.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/1_2ee01678-d570-4704-bef1-08f99aa47b0d.jpg", 
       title: "Best Friend", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 308500000},
 99: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/THE%20SHADE.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/WA_1778890057592.jpeg", 
       title: "THE SHADE", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 223500000},
 100: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Television%20_%20So%20Far%20So%20Good.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/artworks-897VbaCODT1e-0-t500x500.jpg", 
       title: "Television / So Far So Good", 
       artist: "Rex Orange County",
       tags: ["sad","sedih","senang"],
       views: 176900000},
 101: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Pluto%20Projector.mp3", 
       ...
       tags: ["sad","sedih"],
       views: 89500000},
 102: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Sunflower.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/1_a75cf196-0362-4f71-9bf7-78bd17216447.jpg", 
       title: "Sunflower", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 216600000},
 103: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/AMAZING.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/WA_1778890057592.jpeg", 
       title: "AMAZING", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 78500000},
 104: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Untitled.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/artworks-897VbaCODT1e-0-t500x500.jpg", 
       title: "Untitled", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 23700000},
 105: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Always.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/1_ff4e73ba-3eb7-4295-966e-b7539e139c13.jpg", 
       title: "Always", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 10600000},
 106: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/ONE%20IN%20A%20MILLION.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/WA_1778890057592.jpeg", 
       title: "ONE IN A MILLION", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 10500000},
 107: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Never%20Enough.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/artworks-897VbaCODT1e-0-t500x500.jpg", 
       title: "Never Enough", 
       artist: "Rex Orange County",
       tags: ["sad","sedih"],
       views: 8900000},
 108: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Rearrange%20My%20World.mp3", 
       ...
       tags: ["sad","sedih"],
       views: 10700000},
 109: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Loving%20Is%20Easy.mp3", 
       ...
       tags: ["sad","sedih"],
       views: 50900000},
 110: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/There%E2%80%99s%20a%20Field%20That%E2%80%99s%20Only%20Yours.mp3", 
       image: "https://raw.githubusercontent.com/PretyFX69/music-files/refs/heads/main/WA_1778894185659.jpeg", 
       title: "There’s a Field (That’s Only Yours)", 
       artist: "Danial Caesar & Rex Orange County",
       tags: ["sad","sedih"],
       views: 1700000},
  111: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Kicau%20Mania.mp3", 
 ...
 113: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/That%20Should%20Be%20Me.mp3", 
       ...
       views: 194800000},
 114: { audio: "https://github.com/PretyFX69/music-files/raw/refs/heads/main/Beauty%20And%20A%20Beat.mp3", 
