...
async function sendMessage() {
    ...
    try {
        ...
        if (firstUserMsg) saveChatSession(responseStyle, firstUserMsg.content);
...
function saveChatSession(mode, firstMessage) {
    ...
    // Simpan full messages dari chatHistory
    const fullMessages = JSON.parse(JSON.stringify(mode === 'angry' ? chatHistories.worm : chatHistories.normal));
    // Cek apakah sesi pertama sudah sama (update sesi yang sedang berjalan)
    if (sessions.length > 0 && sessions[0].msg === firstMessage) {
        sessions[0].messages = fullMessages;
        sessions[0].ts = Date.now();
        localStorage.setItem(key, JSON.stringify(sessions));
        updateChatPanelHistory();
        return;
    }
    sessions.unshift({ msg: firstMessage, ts: Date.now(), messages: fullMessages });
...
function loadChatSession(key, idx, mode) {
    let sessions = [];
    try { sessions = JSON.parse(localStorage.getItem(key) || '[]'); } catch(e) {}
    ...
    if (needWorm) {
        chatHistories.worm = session.messages;
        chatHistory = chatHistories.worm;
    } else {
        chatHistories.normal = session.messages;
        chatHistory = chatHistories.normal;
    }
    // Render ulang chat
    const container = document.getElementById('chatContainer');
...
function aiUpdateSignInUI() {
    ...
    if (user) {
        ...
        av.textContent = (user.email || 'Z')[0].toUpperCase();
    } else {
...
function aiDoSignIn() {
    ...
    // Cek dari localStorage (akun yg dibuat via sign up)
    let accounts = JSON.parse(localStorage.getItem('ai_accounts') || '{}');
    if (!accounts[email] || accounts[email] !== pw) {
    ...
    setTimeout(() => {
        localStorage.setItem('ai_panel_user', JSON.stringify({ email, name: email.split('@')[0] }));
...
function aiDoSignUp() {
    ...
    let accounts = JSON.parse(localStorage.getItem('ai_accounts') || '{}');
    if (accounts[email]) { showNotification('Email sudah terdaftar', 'error'); return; }
    accounts[email] = pw;
...
const HARDCODED_BACKEND_URL = 'https://property-thou-randy-identifying.trycloudflare.com';
